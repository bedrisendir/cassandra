...
import org.apache.cassandra.db.commitlog.capi.PerfTest;
...
public class CassandraDaemon {
	...
	static {
		...
		SharedMetricRegistries.getOrCreate("logback-metrics").addListener(new MetricRegistryListener.Base() {
			...
			public void onMeterAdded(String metricName, Meter meter) {
				// Given metricName consists of appender name in logback.xml +
				// "." + metric name.
	...
	protected void setup() {
		...
		/*
		 * Migrate pre-3.0 keyspaces, tables, types, functions, and aggregates,
		 * to their new 3.0 storage. We don't (and can't) wait for commit log
		 * replay here, but we don't need to - all schema changes force explicit
		 * memtable flushes.
		 */
		...
		// Re-populate token metadata after commit log recover (new peers might
		// be loaded onto system keyspace #10293)
		StorageService.instance.populateTokenMetadata();

		// migrate any legacy (pre-3.0) hints from system.hints table into the
		// new store
		new LegacyHintsMigrator(DatabaseDescriptor.getHintsDirectory(), DatabaseDescriptor.getMaxHintsFileSize())
				.migrate();
		...
		completeSetup();
		// My little tiny test
		logger.error("Perf Start");
		PerfTest t = new PerfTest();
		t.insert();
		logger.error("Perf End");
	}

	/*
	 * Asynchronously load the row and key cache in one off threads and return a
	 * compound future of the result. Error handling is pushed into the cache
	 * load since cache loads are allowed to fail and are handled by logging.
	 */
	...
	private void logSystemInfo() {
		if (logger.isInfoEnabled()) {
			...
			logger.info("Heap size: {}/{}", FBUtilities.prettyPrintMemory(Runtime.getRuntime().totalMemory()),
	 ...
	 * -specific arguments. To clarify, this is a hook for JSVC.
	 *
	 * @param arguments
